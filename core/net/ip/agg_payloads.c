...
int get_num_aggregated_payloads(){
        //printf("get_num_aggregated_payloads\n");
	//printf("Parsing: Number of agg message %d \n",rcv_payloads.count_payloads);
...
void aggregate_payloads(){
	...
	float alpha;
        ...
        //min_energy goes from 0 to 228
        //If there is no node without harvesting in the route, min energy will go from 0 to 100
        //Else, if there is a node without harvesting in the route, 128 will be added to the value of min_energy
        min_energy = get_min_energy_on_route();
        //printf("min_energy = %u\n", min_energy);
	
	// route_energy goes from 0 to 1.
	min_energy_on_path = get_energy_on_route(); // after this inversion 0 means empty and 1 means full
	//printf("1 - Energy on route is (0-100) %u \n", (unsigned) (min_energy_on_path));
	
	index_agg_function = (unsigned) (min_energy_on_path);
        
        if (min_energy > 100) {
            alpha = 0.0; //when alpha is zero, aggregation is maximized
	...
	//alpha = min_energy_on_path; //linear
        ...
        //printf("alpha - poly = %lu\n", (long unsigned) (alpha * 100));
        // Function the determines how many payloads will be send. Allways more than or equal to 1
        printf("get_num_rcv_payloads = %u\n", get_num_rcv_payloads());
	num_output_payloads = 1 + ((unsigned) (alpha * get_num_rcv_payloads()));
        printf("num_output_payloads = %u\n", ((unsigned) (num_output_payloads)));
        printf("agg_payloads = %u\n", get_num_rcv_payloads() - ((unsigned) (num_output_payloads)));
	...
	//printf("aggregated_payloads.count_payloads = %d\n", aggregated_payloads.count_payloads);
}
