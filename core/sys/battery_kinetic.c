...
short new_day=1;
double total_consumption=0, periodic_consumption=0, total_solar_charging = 0, energy_value;
unsigned battery_has_harvesting = 0;
double energy_balance = 0.0, day_residual = 0.0;
...
void battery_start(clock_time_t period, unsigned seconds, double fixed_perc_energy, unsigned variation, unsigned harvesting_number) {
	...
	batt.qmax = 1*1000000;
	batt.q1_0 = 1*1000000 * random_variable * 0.5;
	batt.q2_0 = 1*1000000 * random_variable * 0.5; // q1 and q2 should start balanced
	batt.k = 0.1;
  ...
  if (node_id == 8 || node_id == 19) {
    battery_has_harvesting = 128; //128 means False and 0 means True
  }

  // if (node_id == 3){
  //   battery_has_harvesting = 0;  
  // } else {
  //   battery_has_harvesting = 128;
  // }
...
void update_battery(){
  ...
  unsigned start_sunlight=6, end_sunlight=start_sunlight+10;// in hours, considering 24h --- Trace is conf to run 13, which means between 8h to 21h
  unsigned duration_solartrace_value=1;// number of timer_counter's units which the solar value longs 
  unsigned has_harvesting;
...
//-------- selects another day profile in the begining of a new day -----------------
  if(new_day==1){
        ...
        day_residual = (batt.q1_0) + (batt.q2_0);
        //printf("Start of the day: %lu\n", (long unsigned) day_residual);
...
has_harvesting = get_has_harvesting();
if (has_harvesting != 128) {
  if((sim_time_minutes>=start_sunlight*60) && (sim_time_minutes<=end_sunlight*60)){// it is in the sun light time
        ...
        //total_solar_charging = total_solar_charging +(solar_charging*time_each_stt[stt_couter]*convert_tick2sec*convert_sec2hour);
  ...
  if(sim_time_minutes>=(24*60)){// end of a day // reset for a new day
	timer_couter=0;
	new_day=1;
	solar_array_couter=0;
        energy_value = (get_battery_charge() / get_max_charge()) * 100;
        //printf("energy_value = %lu\n", (long unsigned) energy_value);
        //energy_balance = energy_value >= 25.0 ? total_solar_charging - total_consumption : 0.0;
        //total_solar_charging = 0.0;
        //total_consumption = 0.0;
        if (energy_value <= 25) {
            energy_balance = 1.0;
        } else if (energy_value >= 75) {
            energy_balance = -1.0;
        } else {
            energy_balance = day_residual - ((batt.q1_0)+(batt.q2_0));
        }
        //printf("End of the day: %lu\n", (long unsigned) energy_balance);
	//printf("Battery: End of day \n"); 
   } /*else if (sim_time_minutes == 0) {
       energy_balance = (batt.q1_0) + (batt.q2_0);
       printf("Start of the day: %lu\n", (long unsigned) energy_balance);
   }*/
}
...
//-------- call the Kinetic model -----------------
  for(stt_couter=0;stt_couter<num_states;stt_couter++){
	...
	// prints
	PRINTF("Battery - Available charge at next time interval is (microAh):%lu\n",(unsigned long) (batt.q1_0));
	PRINTF("Battery:Bound charge at next time interval is (microAh):%lu\n",(unsigned long) (batt.q2_0));
    ...
    // Print
	printf("Log - Residual (microAh):%lu\n",(long unsigned) ((batt.q1_0)+(batt.q2_0)));
        //printf("total_solar_charging = %u\n", (unsigned) total_solar_charging);
	...
	//printf("Energy: CPU ticks %lu, LPM %lu, Tx %lu, Rx %lu \n", stats_com.cpu, stats_com.lpm, stats_com.transmit, stats_com.listen);
        //printf("teste:%lu:%lu:%lu\n", ((long unsigned) batt.q1_0), ((long unsigned) (total_consumption)), ((long unsigned) (periodic_consumption)));
  
}

long double get_battery_charge(){
    return batt.q1_0 + batt.q2_0;
}
